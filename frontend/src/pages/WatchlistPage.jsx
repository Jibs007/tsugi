...
export default function WatchlistPage() {
  ...
  const totalEps = entries
    ...
    .reduce((s, e) => s + (MOCK_ANIME.find((a) => a.id === e.animeId)?.eps || 0), 0);
    ...
    <div style={{ padding: '36px 40px' }} className="animate-fade-in">
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <div style={{ display: 'flex', gap: 24 }}>
          {[['Total', entries.length], ['Episodes', totalEps], ['Completed', entries.filter((e) => e.status === 'completed').length]].map(([l, v]) => (
            <div key={l}>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.map((entry) => {
            ...
            return (
              <div key={entry.animeId} style={{ display: 'flex', alignItems: 'center', gap: 14, background: t.surface, border: `1px solid ${t.border}`, borderRadius: 10, padding: '10px 16px' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                  {Object.entries(STATUS_LABELS).map(([key, label]) => (
                    <button key={key} onClick={() => upsertEntry(entry.animeId, key)} style={{
